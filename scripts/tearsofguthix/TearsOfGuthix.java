...
public class TearsOfGuthix extends EnumScript<State> implements Painting,MessageListening07{
    ...
    private boolean hasStoneBowl = false,
            collectedTears = false;
    ...
    private State getQuestState(){
        if(currentStep == 0){
            ...
            hasStoneBowl = Inventory.find("Stone bowl").length > 0;
            if(hasStoneBowl){
    ...
    public State navigateToTearsCave(){
    ...
    public State handleState(State state) {
        ...
        switch(state){
            ...
            case ENTERING_TEARS_CAVE:
                cave = Objects.findNearest(20,"Tunnel");
            ...
            case CROSSING_STEPPING_STONES:
                ...
                break;
            case CROSSING_CHASM:
                lights = NPCs.findNearest(Filters.NPCs.nameEquals("Light creature").combine(Filters.NPCs.inArea(TEARS_UPPER_AREA), true));
                if(lights.length > 0){
            ...
            case MINING_MAGIC_STONE:
                minable = Objects.findNearest(10,Filters.Objects.nameContains("Rock").combine(Filters.Objects.actionsContains("Mine"),false));
            ...
            case WITHDRAWING_PICKAXE:
                EzBanking.withdraw(1,true,getUsablePickaxes());
                break;
            case WITHDRAWING_CHISEL:
                EzBanking.withdraw(1,true,"Chisel");
                break;
            case WITHDRAWING_BULLSEYE_LANTERN:
                EzBanking.withdraw(1,true,Filters.Items.idEquals(BULLSEYE_LANTERN_LIT_ID,BULLSEYE_LANTERN_UNLIT_ID));
                break;
            case WITHDRAWING_SAPPHIRE_LANTERN:
                EzBanking.withdraw(1,true,"Sapphire lantern");
                break;
            case WITHDRAWING_SAPPHIRE:
                EzBanking.withdraw(1,true,"Sapphire");
                break;
            case WITHDRAWING_TINDERBOX:
                EzBanking.withdraw(1, true, "Tinderbox");
                break;
            case CLOSING_BANK:
    ...
    private boolean hasPickaxe(){
        pickaxe = Inventory.find(Filters.Items.nameEquals(getUsablePickaxes()).combine(Universal.getUnnotedFilter(),false));
        return pickaxe.length > 0;
    }
    private boolean hasChisel(){
        chisel = Inventory.find(Filters.Items.nameEquals("Chisel").combine(Universal.getUnnotedFilter(),false));
        return chisel.length > 0;
    }
    private boolean hasTinderbox(){
        tinderbox = Inventory.find(Filters.Items.nameEquals("Tinderbox").combine(Universal.getUnnotedFilter(),false));
    ...
    private boolean hasSapphire(){
        sapphire = Inventory.find(Filters.Items.nameEquals("Sapphire").combine(Universal.getUnnotedFilter(),false));
    ...
    private boolean hasGamesNecklace(){
        gamesNecklace = Inventory.find(Filters.Items.nameContains("Games necklace"));
    ...
    private boolean hasMagicStone(){
        magicStone = Inventory.find("Magic stone");
