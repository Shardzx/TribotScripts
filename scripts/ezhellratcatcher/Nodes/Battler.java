...
public class Battler extends Node {
    ...
    public void execute() {
        ...
        switch(state){
            ...
            case STARTING_BATTLE:
                Vars.generateWaitingTime();
                boolean shouldClickAnyway = false;
                if(Utilities.hasInterface(193)){ // interface after defeating behemoth
                    RSInterface i = Interfaces.get(193,1);
                    if(i != null){
                        String txt = i.getText();
                        if(txt != null && txt.contains("slays the rat")){
                            Vars.behemothsDefeated++;
                            shouldClickAnyway = true;
                        }
                    }
                }
                if(Game.isUptext("->")){
                ...
                if((!Utilities.isConversing() || shouldClickAnyway) && AccurateMouse.click(Vars.curtain[0], "Enter")){
                ...
                if(chat != null && chat.click()){
                    ...
                    }, 5000) && Timing.waitCondition(EzConditions.isConversing(), 2500)){
    ...
    public void loseItemSelection(){
        String uptext = Game.getUptext();
        if(uptext == null)
            return;
        if(uptext.substring(uptext.length()-3, uptext.length()-1).equals("->")){
            Mouse.click(1);
            return;
        } else {
            // pick other ways to lose uptext.
            int rng = General.random(1,100);
            if(rng > 90){//try clicking under my tile.
