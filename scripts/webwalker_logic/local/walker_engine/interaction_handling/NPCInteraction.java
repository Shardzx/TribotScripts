...
public class NPCInteraction implements Loggable {
    ...
    public static boolean talkTo(Filter<RSNPC> rsnpcFilter, String[] talkOptions, String[] replyAnswers) {
        ...
        handleConversation(replyAnswers, false);
        return true;
    }

    public static boolean clickNpcAndWaitChat(RSNPC npc, String[] action) {
		return InteractionHelper.click(npc, action) && waitForConversationWindow();
	}
    ...
    public static void handleConversation(){
        handleConversation(new String[]{}, false);
    }
    
    public static void handleConversation(String[] options){
        handleConversation(options, false);
    }

    public static void handleConversation(String[] options, boolean combat){
    	List<String> blackList = new ArrayList<String>();
    	int countFailed = 0;
        while (true){
        	if(Game.getGameState() < 30)
        		break;
            if (!combat && WaitFor.condition(General.random(650, 800), () -> isConversationWindowUp() ? WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE) != WaitFor.Return.SUCCESS){
                break;
            }
