...
public class ACamera 
{
    ...
    public void setCameraAngle(int angle, int sleepTime){
    	synchronized(angleThread)
    	{
	    	if(!angleThread.isAlive())
	    		angleThread.start();
	    	
	    	angleThread.sleepTime = sleepTime + calculateSleepTime();
	    	angleThread.angle = angle;
	    	angleThread.notify();
    	}
    }
    
    public void setCameraRotation(int rotation, int sleepTime){
    	synchronized(rotationThread)
    	{
	    	if(!rotationThread.isAlive())
	    		rotationThread.start();
	    	
	    	rotationThread.sleepTime = sleepTime;
    ...
    public void setCameraRotation(int rotation)
    {
    	synchronized(rotationThread)
    	{
	    	if(!rotationThread.isAlive())
	    		rotationThread.start();
	    	rotationThread.sleepTime = 0;
	    	rotationThread.rotation = rotation;
	    	rotationThread.notify();
    	}
    }
    
    public void setCamera(int angle, int rotation){
    	setCameraAngle(angle);
    	setCameraRotation(rotation);
    }
    
    public void setCamera(int angle, int rotation, int sleep){
    	setCameraAngle(angle,sleep);
    	setCameraRotation(rotation,sleep);
    }
